...
const comparablePage = (value: ComparablePageInput) => ({
  title: value.title ?? '',
  slug: value.slug ?? '',
...
const sameImportedPage = (existing: ComparablePageInput, target: ComparablePageInput): boolean =>
  JSON.stringify(comparablePage(existing)) === JSON.stringify(comparablePage(target))
  ...
  for (const page of pages) {
    ...
    if (existing.docs[0]) {
      if (sameImportedPage(existing.docs[0], data)) {
        unchanged += 1
        console.log(`unchanged wp:${page.wordpressId}`)
        continue
      }
      ...
      updated += 1
      console.log(`updated wp:${page.wordpressId} -> payload:${existing.docs[0].id}`)
    } else {
      ...
      created += 1
      console.log(`created wp:${page.wordpressId} -> payload:${createdPage.id}`)
    }
  }
