...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE "payload_mcp_api_keys" (
  	...
  	"topic_candidates_find" boolean DEFAULT false,
  	"topic_candidates_create" boolean DEFAULT false,
  	"topic_candidates_update" boolean DEFAULT false,
  	"evidence_find" boolean DEFAULT false,
  	"evidence_create" boolean DEFAULT false,
  	"evidence_update" boolean DEFAULT false,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"enable_a_p_i_key" boolean,
  	"api_key" varchar,
